...
BEGIN
    ...
    IF project_id_type IS NULL THEN
        RAISE EXCEPTION 'Cannot determine projects.id type for schedule migration';
    END IF;

    SELECT format_type(a.atttypid, a.atttypmod)
      INTO contractor_id_type
      FROM pg_attribute a
      JOIN pg_class c ON c.oid = a.attrelid
      JOIN pg_namespace n ON n.oid = c.relnamespace
     WHERE c.relname = 'contractors'
       AND a.attname = 'id'
       AND a.attnum > 0
       AND NOT a.attisdropped
     LIMIT 1;

    IF contractor_id_type IS NULL THEN
        RAISE EXCEPTION 'Cannot determine contractors.id type for schedule migration';
    END IF;

    EXECUTE format($ddl$
        CREATE TABLE IF NOT EXISTS schedule_tasks (
            id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            project_id %s NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
            parent_task_id UUID REFERENCES schedule_tasks(id) ON DELETE CASCADE,
            contractor_id %s REFERENCES contractors(id) ON DELETE SET NULL,
            wbs_code VARCHAR(50),
            name TEXT NOT NULL,
    ...
    $ddl$, project_id_type, contractor_id_type);
