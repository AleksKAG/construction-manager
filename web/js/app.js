...
class ConstructionManagerUI {
  constructor() {
    ...
    this.templatePage = 1;
    this.templateSearch = '';
    this.selectedTemplates = { tep: 'tep', designSchedule: 'design_schedule' };
    this.bind();
    this.bootstrap();
  }
  ...
  async renderTemplateScreen(defaultCode, title) {
    ...
    const code = this.selectedTemplates[this.currentView] || defaultCode;
    ...
    this.selectedTemplates[this.currentView] = code;
  ...
  async openTemplatePicker(preferredCode) {
    ...
    document.getElementById('modalBody').innerHTML = `<div class="form-grid">${(filtered.length ? filtered : templates).map(t => `<label><input type="radio" name="template_code" value="${t.code}" ${t.code === (this.selectedTemplates[this.currentView] || preferredCode) ? 'checked' : ''}> ${t.name} <span class="metric">(${t.code})</span></label>`).join('')}</div>`;
    this.openModal();
  }
  ...
  async handlePrimaryAction() {
    if (this.currentView === 'tep') {
      const code = this.selectedTemplates.tep || 'tep';
      const tpl = await getTemplate(code);
      return this.openTemplateForm(tpl, null);
    }
    if (this.currentView === 'designSchedule') {
      const code = this.selectedTemplates.designSchedule || 'design_schedule';
      const tpl = await getTemplate(code);
      return this.openTemplateForm(tpl, null);
    }
  }
  ...
  async handleSecondaryAction() {
    ...
    if (this.currentView === 'tep' || this.currentView === 'designSchedule') {
      const code = this.selectedTemplates[this.currentView] || (this.currentView === 'tep' ? 'tep' : 'design_schedule');
  ...
  async handleSaveModal() {
    ...
    if (this.modalMode === 'selectTemplate') {
      ...
      this.selectedTemplates[this.currentView] = selected.value;
      this.closeModal();
