...
1. **Переключить runtime на PostgreSQL (обязательно)** ✅
   - ✅ `cmd/api/main.go`: заменён `sqlite.Open(...)` на `postgres.Open(...)`.
   - ✅ Конфиг: переход на `DATABASE_URL` как основной источник подключения.
...
2. **Доделать контейнерный контур под PostgreSQL (обязательно)** ✅
   - ✅ `Dockerfile`, `docker-compose.yml`, `entrypoint.sh`: убран SQLite-путь и синхронизация db-файла.
   - ✅ Добавлен `depends_on`/ожидание готовности PostgreSQL перед стартом API.
   - ✅ Настроены резервные копии PostgreSQL в S3 (`scripts/backup_postgres_to_s3.sh`).
...
4. **Закрыть технический долг после switch-over (желательно в тот же заход)** ✅
   - ✅ Удалены пакеты и упоминания SQLite из кода, `.env.example`, README.
   - ⏭️ Проверить/добавить индексы и уникальные ограничения на горячих таблицах.
   - ✅ Критерий готовности: в зависимостях и runtime-конфиге больше нет SQLite.

5. **Подготовить безопасный rollout (желательно)** ✅
   - ✅ Зафиксирован backup + rollback runbook в `docs/S3_SETUP.md`.
   - ⏭️ Прогнать smoke после миграции: `health`, проекты, задачи, шаблоны, реестры, workforce.
